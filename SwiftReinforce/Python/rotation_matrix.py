...
if __name__ == "__main__":
    # Test the code
    myoutput = CoordRotationInv( [1.5,4.3], [5,5], np.pi/4 )
    print(myoutput)

    myoutput = LinIntIndicesAndWeights( [10.4,40.3], [5,5] )
    print(myoutput)

    myoutput = ToLinearIndex( [2,0], [5,5])
    print(myoutput)

    myoutput = RotationOperatorMatrix([31, 31], np.pi / 4)
    np.savetxt('matrix.txt', myoutput)
    np.save('matrix.npy', myoutput)
    print(myoutput[400])

    help(CoordRotationInv)
    help(LinIntIndicesAndWeights)
    help(ToLinearIndex)
    help(RotationOperatorMatrix)
    help(MultiRotationOperatorMatrix)
